...
namespace OOPSharp // Note: actual namespace depends on the project name.
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var minTotalIncome = vehicles.Min(x => x.TotalIncome);
            var maxTotalIncome = vehicles.Max(x => x.TotalIncome);

            Console.WriteLine($"Min Total Income : {minTotalIncome}, Max Total Income : {maxTotalIncome}");

            var query = vehicles.Where(x => x.TotalIncome > minTotalIncome && x.TotalIncome < maxTotalIncome)
                                .Select(vh =>
                                 new {
